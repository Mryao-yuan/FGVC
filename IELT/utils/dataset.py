...
class Inaturalist_21_plante(VisionDataset):
    def __init__(self, root, is_train=True, trans=""):
        data_len = None
        ...
        test_img_path = os.path.join(self.root, "test")
        # 标签文件
        train_label_object = open(os.path.join(self.root, "train.txt"))
        test_label_object = open(os.path.join(self.root, "test.txt"))

        if is_train:
            for line in train_label_object:
                train_img_label.append(
                    [
                        os.path.join(train_img_path, line[:-1].split(" ")[0]),
                        int(line[:-1].split(" ")[1]),
                    ]
                )
    ...
    def __getitem__(self, index):
        if self.is_train:
            img, target = (
                imageio.imread(self.train_img_label[index][0]),
            ...
            img, target = (
                imageio.imread(self.test_img_label[index][0]),
...
class OxfordFlowers(Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        train_set = pd.read_csv(
            os.path.join(self.root, "train.txt"), sep=" ", names=["img_path", "target"]
        )
        test_set = pd.read_csv(
            os.path.join(self.root, "test.txt"), sep=" ", names=["img_path", "target"]
        )
        if train:
            self.data = train_set
        else:
...
class OxfordIIITPet(VisionDataset):
    ...
    def __init__(
        ...
        self._images = [
            os.path.join(self._images_folder, f"{image_id}.jpg")
            for image_id in image_ids
        ]
        self._segs = [
            os.path.join(self._segs_folder, f"{image_id}.png") for image_id in image_ids
        ]
