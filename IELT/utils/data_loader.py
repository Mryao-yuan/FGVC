...
def build_transforms(config):
    ...
    if config.data.no_crop:
        train_base = [
            transforms.Resize(
                (config.data.img_size, config.data.img_size), InterpolationMode.BICUBIC
            ),
        ...
        train_base = [
            transforms.Resize((resize, resize), InterpolationMode.BICUBIC),
    ...
    to_tensor = [
        ...
        transforms.Normalize(
            normalized_info["common"][:3], normalized_info["common"][3:]
        ),
    ]
...
def build_loader(config):
    ...
    if config.data.dataset == "cub":
        train_set = Inaturalist_21_plante(config.data.data_root, True, train_transform)
        test_set = Inaturalist_21_plante(config.data.data_root, False, test_transform)
        num_classes = 500
    ...
    mixup_fn = None
    mixup_active = config.data.mixup > 0.0 or config.data.cutmix > 0.0
    if mixup_active:
        mixup_fn = Mixup(
            mixup_alpha=config.data.mixup,
            cutmix_alpha=config.data.cutmix,
            label_smoothing=config.model.label_smooth,
