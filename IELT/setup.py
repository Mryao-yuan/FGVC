...
cfg_file = os.path.join("cub.yaml")
config = SetupConfig(config, "cub.yaml")
config.defrost()
...
config.cuda_visible = "3"

# Environment Settings
config.data.log_path = os.path.join(
    config.misc.output,
    config.misc.exp_name,
    config.misc.log_name + time.strftime(" %m-%d_%H-%M", time.localtime()),
)
