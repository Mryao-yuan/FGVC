...
class RACNN(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=True):
        ...
        self.apn1 = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(1024, 3),  # 类似通道注意力，输出三个值(tx,ty,tl)
            nn.Sigmoid(),
        )
        # 输出t_x,t_y,t_l,形状
    ...
    def forward(self, x):
        ...
        atten1 = self.apn1(
            self.atten_pool(conv5_4).view(-1, 512 * 14 * 14)
        )  # 生成建议区域[tx,ty,tl]
...
class AttentionCropFunction(torch.autograd.Function):
    ...
    def forward(self, images, locs):
        ...
        for i in range(images.size(0)):  # 迭代通道
            ...
            xamp = F.interpolate(
                before_upsample, size=(224, 224), mode="bilinear", align_corners=True
            )  # 双线性插值来放大 attend region
            ret.append(xamp.data.squeeze())

        ret_tensor = torch.stack(ret)
        self.save_for_backward(
            images, ret_tensor
        )  # 保存图像和映射map用于backward==>使用 self.saved_variabkes 来获取
        return ret_tensor
    ...
    def backward(self, grad_output):
        ...
        )  # saved_variables 不用，使用saved.tensors
        in_size = 224
        ...
        ml = (
            ((x < short_size) + (x >= long_size) + (y < short_size) + (y >= long_size))
            > 0
        ).float() * 2 - 1
        # batch下的x,y,l导数形式
        ...
        # 返回三个位置值
        ret[:, 0] = (
            (norm * mx_batch).sum(dim=1).sum(dim=1)
        )  # (用范数*导数图来获取向哪边优化)长宽求和，获取位置
...
if __name__ == "__main__":
    ...
    print(
        " [*] Loss cls:", loss_cls
    )  # [tensor(5.3305, device='cuda:0', grad_fn=<NllLossBackward0>), tensor(5.2256, device='cuda:0', grad_fn=<NllLossBackward0>), tensor(5.3766, device='cuda:0', grad_fn=<NllLossBackward0>)]
    print(
        " [*] Loss rank:", loss_rank
